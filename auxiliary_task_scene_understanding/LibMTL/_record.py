...
class _PerformanceMeter(object):
    def __init__(self, task_dict, multi_input, base_result=None):
        ...
        self.best_result = {"improvement": -1e2, "epoch": 0, "result": 0}

        self.losses = {task: self.task_dict[task]["loss_fn"] for task in self.task_name}
        self.metrics = {
            task: self.task_dict[task]["metrics_fn"] for task in self.task_name
        }
        ...
        with torch.no_grad():
            if task_name is None:
                for tn, task in enumerate(self.task_name):
                    cast(AbsMetric, self.metrics[task]).update_fun(
                        preds[task], gts[task]
                    )
            else:
                cast(AbsMetric, self.metrics[task_name]).update_fun(preds, gts)
    ...
    def display_best_result(self):
        print("=" * 40)
        print(
            "Best Result: Epoch {}, result {}".format(
                self.best_result["epoch"], self.best_result["result"]
            )
        )
        print("=" * 40)
