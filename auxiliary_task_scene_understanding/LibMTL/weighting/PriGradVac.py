...
class PriGradVac(AbsWeighting):
    ...
    def backward(self, losses, **kwargs):
        ...
        for tn_i in range(self.task_num):
            if tn_i == pri_idx:
                continue
            rho_ij = torch.dot(pc_grads[tn_i], grads[pri_idx]) / (
                pc_grads[tn_i].norm() * grads[pri_idx].norm()
            )
            if rho_ij < self.rho_T[tn_i, pri_idx]:
                w = (
                    pc_grads[tn_i].norm()
                    * (
                        self.rho_T[tn_i, pri_idx] * (1 - rho_ij**2).sqrt()
                        - rho_ij * (1 - self.rho_T[tn_i, pri_idx] ** 2).sqrt()
                    )
                    / (
                        grads[pri_idx].norm()
            ...
            # Fix bug of LibMTL
            self.rho_T[tn_i, pri_idx] = (1 - beta) * self.rho_T[
                tn_i, pri_idx
            ] + beta * rho_ij
