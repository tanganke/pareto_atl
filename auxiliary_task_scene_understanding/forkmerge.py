...
def parse_args(parser):
    ...
    parser.add_argument(
        "--train_bs", default=8, type=int, help="batch size for training"
    )
...
def main(params):
    ...
    class NYUtrainer(Trainer):
        ...
        def test(self, test_dataloaders, epoch=None, mode="test"):
            ...
            self.meter.reinit()
            results = {
                task_name: results[task_name] for task_name in params.target_tasks
            }
            return results
    ...
    for aux_task_name in params.source_tasks:
        if aux_task_name not in params.target_tasks:
            ...
            trainer.train(
                nyuv2_train_loader, nyuv2_val_loader, 0, params.pruning_epochs
            )
    ...
    while epoch_start < params.n_epochs:
        ...
        for i, (name, _) in enumerate(ranked_performances[1:]):
            ...
            for alpha in params.alphas:
                ...
                new_result = target_trainer.test(
                    nyuv2_val_loader, epoch_end, mode="val"
                )

                improvement = count_improvement(
                    target_trainer.base_result, new_result, target_trainer.weight
                )
        ...
        print(
            "Epoch: {} Result: {} Best lambda: {} Best criteria: {}".format(
                epoch_end, criteria_dict, lambda_dict, best_improvement
            )
        )
        ...
        if best_improvement > overall_best_improvement:
            ...
            torch.save(
                target_trainer.model.state_dict(), logger.get_checkpoint_path("best")
            )
