...
def parse_args(parser):
    ...
    parser.add_argument(
        "--tasks", default=["segmentation", "depth", "normal"], nargs="+"
    )
...
def main(params):
    ...
    class NYUtrainer(Trainer):
        ...
        def train(
            self, train_dataloaders, val_dataloaders, epochs, return_weight=False
        ):
            ...
            for epoch in range(epochs):
                ...
                self.meter.reinit()
                torch.save(
                    self.model.state_dict(), logger.get_checkpoint_path("latest")
                )
    ...
    if params.pretrained is not None:
        ...
        NYUmodel.model.load_state_dict(
