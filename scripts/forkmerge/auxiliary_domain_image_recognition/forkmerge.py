...
class BaseMultiTaskSampler(metaclass=abc.ABCMeta):
    ...
    def pop(self):
        "Abstract method to pop a task."
        raise NotImplementedError()

    def iter(self):
        "Iterator method to yield tasks. By default, it yields the result of the pop method."
        yield self.pop()
...
class SpecifiedProbMultiTaskSampler(BaseMultiTaskSampler):
    ...
    def __init__(
        self,
        task_dict: Dict[str, DataLoader],
        rng: Union[int, np.random.RandomState],  # default: 0
        ...
        self.unweighted_probs_arr = np.array(
            [task_to_unweighted_probs[k] for k in self.task_names]
        )
        self.task_p = self.unweighted_probs_arr / self.unweighted_probs_arr.sum()

    def pop(self) -> Tuple[str, DataLoader]:
...
def main(args):
    ...
    heads = nn.ModuleDict(
        {
            dataset_name: nn.Linear(backbone.out_features, num_classes)
    ...
    for aux_task_name in args.source:
        if aux_task_name not in args.target:
            ...
            trainer = Trainer(
                "ranking_{}".format(aux_task_name),
                model,
                logger.get_checkpoint_path(aux_task_name),
                args.optimizer,
                args.lr,
                args.momentum,
                args.wd,
            ...
            trainer.train(
                train_loaders, 0, args.pruning_epochs, args.iters_per_epoch, val_loaders
            )
            performance_dict[aux_task_name] = sum(trainer.best_acc1.values()) / len(
                trainer.best_acc1
            )

    ranked_performances = sorted(
        performance_dict.items(), key=lambda kv: kv[1], reverse=True
    )
    ...
    for topk in args.topk:
        ...
        for task_name in task_names:
            new_task_weights[task_name] = 1.0 / topk
        print("top {}: {}".format(topk, new_task_weights))

        trainers[topk] = Trainer(
            "top_{}".format(topk),
            deepcopy(model),
            logger.get_checkpoint_path("top_{}".format(topk)),
            args.optimizer,
            args.lr,
            args.momentum,
            args.wd,
    ...
    while epoch_start < args.epochs:
        ...
        for name, trainer in trainers.items():
            ...
            trainer.train(
                train_loaders, epoch_start, epoch_end, args.iters_per_epoch, val_loaders
            )
            trainer.load_best()
            theta_dict[name] = deepcopy(trainer.model.state_dict())
            performance_dict[name] = sum(trainer.best_acc1.values()) / len(
                trainer.best_acc1
            )
        ...
        print(ranked_performances)
        lambda_dict = {name: 0.0 for name, _ in ranked_performances}
        ...
        print(
            "Epoch: {} Result: {} Best lambda: {} Best criteria: {}".format(
                epoch_end, criteria_dict, lambda_dict, best_criteria
            )
        )
